...
from upwork_profile_create.artifacts import ensure_artifacts_dir, resolve_fill_plan_path, resolve_report_path
from upwork_profile_create.models import FILL_MODES
from upwork_profile_create.composer import compose_fill_plan
from upwork_profile_create.loader import list_profiles, load_inputs
from upwork_profile_create.mapper import list_form_mappings
from upwork_profile_create.validator import validate_fill_plan, validate_fill_plan_file
from upwork_profile_create.writer import build_fill_report, write_fill_plan, write_fill_report


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Upwork profile create CLI for fill-plan.")
    ...
    artifact_path_parser = subparsers.add_parser(
        "artifact-path",
        help="Resolve fill-plan artifact path for profile title.",
    )
    artifact_path_parser.add_argument("--profile-title", required=True, help="Profile title.")

    subparsers.add_parser("form-mappings", help="List Upwork form field mappings.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "compose":
        ...
        output = args.output
        if output is None:
            output = str(resolve_fill_plan_path(fill_plan.profile.profile_title))
    ...
    if args.command == "artifact-path":
        slug = slugify_target_role(args.profile_title)
        path = resolve_fill_plan_path(args.profile_title)
    ...
    if args.command == "write-report":
        ...
        output = args.output
        if output is None:
            output = str(resolve_report_path(fill_plan.profile.profile_title))
